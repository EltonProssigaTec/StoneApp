...
## 11. Sincronização com app.json (Expo)

**IMPORTANTE:** O Expo gera os arquivos Gradle automaticamente através do `app.json`. Para garantir que o build funcione corretamente, certifique-se de que o `app.json` esteja sincronizado:

### Configurações Críticas no app.json
...
Sempre que atualizar a versão do app, verifique:

1. ✅ `version` no app.json = `versionName` no build.gradle
2. ✅ `versionCode` no app.json = `versionCode` no build.gradle
3. ✅ `package` no app.json = `applicationId` no build.gradle
4. ✅ Após alterar app.json, rode: `npx expo prebuild --clean`

### Evite Edição Manual

- ❌ **NÃO** edite manualmente `android/app/build.gradle` (será sobrescrito)
- ❌ **NÃO** edite manualmente `AndroidManifest.xml` (será sobrescrito)
- ✅ **SEMPRE** faça alterações através do `app.json` e plugins do Expo
- ✅ **EXCEÇÃO**: `signingConfigs` pode ser editado manualmente no build.gradle

---

## 12. Histórico de Alterações

**2025-11-06 (Atualização 2)**
- Corrigido versionCode no app.json (61 → 62)
- Removida configuração inválida `android.config`
- Corrigidas permissões duplicadas
- Adicionada seção de sincronização com app.json
